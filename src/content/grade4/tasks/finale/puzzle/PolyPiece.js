...
export class PolyPiece {
    constructor(container, options) {
        ...
        this._handlers = []; // Eseménykezelők követése a biztonságos takarításhoz
    ...
    destroy() {
        // Összes eseménykezelő eltávolítása (Rule 104)
    ...
    initEvents() {
        this._addListener(this.element, 'mousedown', this.handleMouseDown.bind(this));
        
        this._addListener(this.element, 'mousemove', (e) => {
        ...
        this._addListener(this.element, 'mouseleave', () => { 
            if (!this.isDragging) { 
    ...
    handleMouseDown(e) {
        ...
        // Regisztráljuk a window handlereket is a takarításhoz
        this._handlers.push({ target: window, type: 'mousemove', handler: this._windowMouseMove });
        this._handlers.push({ target: window, type: 'mouseup', handler: this._windowMouseUp });
    }
    ...
    handleWindowMouseUp() {
        ...
        // Eltávolítjuk a window handlereket a regiszterből is
        this._handlers = this._handlers.filter(h => h.handler !== this._windowMouseMove && h.handler !== this._windowMouseUp);
